...
class SocketLineReader {
  ...
  onData(chunk) {
    ...
    for (let newline = this.buffer.indexOf(0x0A); newline !== -1; newline = this.buffer.indexOf(0x0A)) {
      ...
      else if (this.lines.push(line) > MAX_RESPONSE_LINES) {
...
class PowerZoneClient {
  constructor(host, port = 7621, { timeoutMs = 5000 } = {}) {
  ...
  async validatedAddresses() {
    ...
    if (!addresses.length || addresses.some(({ address }) => !isLocalAddress(address))) {
      throw new PowerZoneNetworkError(
        "PowerZone's unauthenticated API requires a private or link-local address",
      );
    }
    return addresses;
  }
    ...
    try {
      for (const command of commands) {
        ...
        while (true) {
          ...
          if (++lines > MAX_RESPONSE_LINES) throw new PowerZoneApiError('PowerZone response is too large');
          if (line === `*${command}`) {
          ...
          if (line.startsWith('#')) {
            ...
            throw new PowerZoneApiError(
              `PowerZone rejected ${command}: ${separator === -1 ? line.slice(1) : line.slice(separator + 1)}`,
            );
          }
          if (line.startsWith('+')) {
            const separator = line.indexOf(' ');
            if (separator !== -1) values[line.slice(1, separator)] = decodeValue(line.slice(separator + 1));
          }
        }
      }
      return responses;
      ...
      throw new PowerZoneApiError(`PowerZone communication failed: ${error.message}`);
    } finally {
      socket.destroy();
    }
  }
  ...
  async validateServer() {
    const [api, device, outputs, eqBands] = await this.execute([
      'GET API_VERSION', 'GET SYSTEM.DEVICE.*', 'GET OUT.COUNT', 'GET OUT.EQ.COUNT',
    ]);
    const info = {
    ...
    if (!info.apiVersion || !info.serial || !Number.isInteger(info.outputs) || !Number.isInteger(info.eqBands)
      || info.outputs < 1 || info.outputs > MAX_OUTPUTS || info.eqBands < 1 || info.eqBands > MAX_EQ_BANDS) {
      throw new PowerZoneApiError('The device did not return usable PowerZone output EQ registers');
    }
  ...
  async outputNames() {
    ...
    const [response] = await this.execute(['GET OUT-*.NAME']);
    return Object.fromEntries(Array.from({ length: info.outputs }, (_, index) => {
      const id = index + 1;
      return [id, String(response[`OUT-${id}.NAME`] ?? `Output ${id}`)];
    }));
  }

  validateOutput(outputId) {
    const id = Number(outputId);
    if (!Number.isInteger(id) || id < 1 || id > this.deviceInfo.outputs) {
      throw new PowerZoneApiError(`Output ${outputId} is outside this amplifier's range`);
    }
    return id;
  }
  ...
  async applyPreset(outputId, name) {
    const info = this.deviceInfo || await this.validateServer();
    const id = this.validateOutput(outputId);
    const preset = getPreset(name);
    const root = `OUT-${id}.EQ`;
    const bands = powerZoneBands(preset.id, info.eqBands);
    if (!bands.length) {
      await this.execute([`SET ${root}.BYPASS 1`]);
      return preset;
    }
    ...
    for (let band = bands.length + 1; band <= info.eqBands; band += 1) {
      commands.push(`SET ${root}-${band}.BYPASS 1`);
    }
  ...
  async readPreset(outputId) {
    ...
    for (let band = 1; band <= info.eqBands; band += 1) {
      const prefix = `${root}-${band}`;
      commands.push(
        `GET ${prefix}.TYPE`, `GET ${prefix}.FREQ`, `GET ${prefix}.Q`,
        `GET ${prefix}.GAIN`, `GET ${prefix}.BYPASS`,
      );
    }
    const values = Object.assign({}, ...await this.execute(commands));
    if (asBoolean(values[`${root}.BYPASS`])) return getPreset('flat');
    for (const preset of PRESETS) {
      if (preset.id !== 'flat' && PowerZoneClient.matchesBands(
        root, powerZoneBands(preset.id, info.eqBands), info.eqBands, values,
      )) return preset;
    }
    return null;
  }
  ...
  static matchesBands(root, expected, bandCount, values) {
    for (let band = 1; band <= bandCount; band += 1) {
      const prefix = `${root}-${band}`;
      if (band > expected.length) {
        if (!asBoolean(values[`${prefix}.BYPASS`])) return false;
      } else {
        const target = expected[band - 1];
        const frequency = Number(values[`${prefix}.FREQ`]);
        const q = Number(values[`${prefix}.Q`]);
        const gain = Number(values[`${prefix}.GAIN`]);
        if (asBoolean(values[`${prefix}.BYPASS`]) || values[`${prefix}.TYPE`] !== target.type
          || !Number.isFinite(frequency) || Math.abs(frequency - target.frequency) > 0.1
          || !Number.isFinite(q) || Math.abs(q - target.q) > 0.01
          || !Number.isFinite(gain) || Math.abs(gain - target.gain) > 0.05) return false;
      }
    }
    return true;
  }
...
module.exports = { PowerZoneApiError, PowerZoneClient, isLocalAddress };
