...
const PRESET_BY_ID = new Map(PRESETS.map(preset => [preset.id.toLowerCase(), preset]));
const PRESET_BY_TITLE = new Map(PRESETS.map(preset => [preset.title.toLowerCase(), preset]));

function getPreset(name) {
  const normalized = String(name || '').trim().toLowerCase();
  const preset = PRESET_BY_ID.get(normalized) || PRESET_BY_TITLE.get(normalized);
  if (!preset) throw new RangeError(`Unknown Sonance EQ preset: ${name}`);
  return preset;
}
...
function powerZoneBands(name, bandCount) {
  ...
  if (!gains.some(gain => gain !== 0)) return [];
  ...
  if (count === 1) {
    ...
    const ratio = 2 ** ((ISO_CENTERS.length - 1) / (count - 1));
...
module.exports = { PRESETS, getPreset, powerZoneBands };
